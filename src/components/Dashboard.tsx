...
export function Dashboard() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#fafafa] dark:bg-[#020817] flex selection:bg-primary/30 transition-colors duration-1000">
      {isLoading ? (
        ...
        <>
          ...
          <main className="flex-1 relative z-10">
            <header className="h-24 px-8 lg:px-16 flex items-center justify-between sticky top-0 bg-background/50 backdrop-blur-xl z-40 border-b border-white dark:border-white/5">
              <motion.div style={{ y: headerY, opacity: headerOpacity }} className="lg:hidden">
              ...
              <div className="flex items-center gap-6 ml-auto">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button
                    variant="outline"
                    className="rounded-full px-8 h-12 font-bold border-slate-200 dark:border-white/10 hover:bg-primary hover:text-white hover:border-primary transition-all shadow-sm"
                    onClick={handleShare}
                  >
            ...
            <div className="px-8 lg:px-16 py-12 lg:py-24 max-w-7xl mx-auto">
              <motion.div 
              ...
              >
                <div className="mb-20 space-y-4">
                  <span className="text-sm font-black uppercase tracking-[0.4em] text-primary block reveal-text">Digital Vault</span>
                  <h1 className="text-6xl lg:text-[7rem] font-serif font-black tracking-tighter leading-[0.9] text-slate-900 dark:text-white">
                  ...
                  <div className="lg:col-span-8">
                    {activeTab === "inbox" && (
                      <div className="space-y-12">
                        {myTestimonials.length === 0 ? (
                          <div className="group relative p-20 lg:p-32 rounded-[64px] bg-white/50 dark:bg-white/2 backdrop-blur-3xl border-2 border-dashed border-slate-200 dark:border-white/10 text-center overflow-hidden">
                            ...
                            <motion.div 
                              ...
                              className="bg-slate-100 dark:bg-white/5 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-10 relative z-10"
                            >
                              <MessageSquare className="w-10 h-10 text-slate-300 dark:text-slate-600" />
                            </motion.div>
                            <h2 className="text-4xl lg:text-5xl font-serif font-black mb-6 text-slate-900 dark:text-white relative z-10">Your story begins here.</h2>
                            <p className="text-xl text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-12 font-medium leading-relaxed relative z-10">
                            ...
                            <motion.button 
                              ...
                              whileTap={{ scale: 0.95 }}
                              onClick={handleShare}
                              className="px-12 py-6 bg-primary text-white rounded-full font-black text-xl shadow-2xl shadow-primary/30 relative z-10 glow-primary-subtle"
                            >
                              Launch My Capsule
                            </motion.button>
                          </div>
                    ...
                    {activeTab === "profile" && (
                      <motion.div 
                        ...
                        className="p-12 lg:p-20 rounded-[64px] bg-primary text-white relative overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)]"
                      >
                        <div className="absolute top-0 right-0 p-40 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
                        <div className="absolute bottom-0 left-0 p-40 bg-indigo-500/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2" />
                        
                        <div className="relative z-10 flex flex-col md:flex-row gap-12 items-center text-center md:text-left">
                          <Avatar className="h-48 w-48 border-8 border-white/20 shadow-2xl">
                            <AvatarImage src={me.imageUrl} />
                            <AvatarFallback className="text-6xl font-black">{me.name[0]}</AvatarFallback>
                          </Avatar>
                          <div className="space-y-6">
                            <div className="space-y-2">
                              <h2 className="text-6xl font-serif font-black tracking-tight">{me.name}</h2>
                              <p className="text-2xl font-bold opacity-80">{me.college} &apos;{me.batchYear?.slice(-2)}</p>
                            </div>
                            <div className="flex gap-4 justify-center md:justify-start">
                              <Button size="lg" className="rounded-full px-8 bg-white text-primary hover:bg-white/90 font-black h-14" onClick={() => setIsEditingProfile(true)}>Edit Identity</Button>
                              <Button size="lg" variant="ghost" className="rounded-full px-8 text-white hover:bg-white/10 font-bold h-14" onClick={handleShare}>Share Link</Button>
                            </div>
                          </div>
                        </div>
                      </motion.div>
                    ...
                    <div className="sticky top-32">
                      ...
                      {activeTab === "inbox" && (
                        <motion.div 
                          ...
                          className="mt-12 p-8 rounded-[40px] bg-slate-900 text-white relative overflow-hidden group hover:scale-[1.02] transition-transform duration-500"
                        >
                          <Sparkles className="w-10 h-10 text-primary mb-6 group-hover:rotate-12 transition-transform" />
                          <h4 className="text-2xl font-serif font-black mb-4 tracking-tight">Your Legacy.</h4>
